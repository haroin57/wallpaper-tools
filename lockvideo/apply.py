...
def write_index(pairs, state, sysdef):
    ...
    idx["Displays"] = {d: {"Linked": linked(template, active_slot(pairs, state, d)), "Type": "linked"}
                        for d in pairs}
    # Spaces は WallpaperAgent の権威データ。ここを書かないと再起動時に Displays を巻き戻される。
    # （単一ディスプレイ時は Spaces['']['Displays'][uuid] が実画面へ直接適用される。LastSet/LastUse
    #  等の付随キーを保つため、既存 Linked を template にして assetID だけ差し替える。）
    for _sk, sv in (idx.get("Spaces") or {}).items():
        dfl = sv.get("Default")
