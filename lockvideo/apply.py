...
# 新スキーマ(macOS 26 Tahoe: Desktop/Idle シーンモデル)で書き込むシーン。lockvideo はロック画面
# 担当なので既定は Idle のみ。デスクトップ壁紙も動画にしたいなら ("Desktop", "Idle") にする
# （ただしデスクトップは通常 MonitorWall が別途担当する）。旧 Linked スキーマでは未使用。
SCENE_KEYS = ("Idle",)
